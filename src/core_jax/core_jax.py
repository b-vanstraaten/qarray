...
def return_analytical_solution_open(cdd_inv: CddInv, cgd: Cgd, vg: VectorList) -> VectorList:
    return cgd @ vg


def compute_analytical_solution_open(cdd_inv: CddInv, cgd: Cgd, vg):
    analytical_solution = cgd @ vg
    function_list = [
        partial(return_analytical_solution_open, cdd_inv=cdd_inv, cgd=cgd, vg=vg),
...
def ground_state_open_jax(vg: VectorList, cgd: Cgd, cdd_inv: CddInv) -> VectorList:
    """
        ...
        """
    analytical_solution = jax.vmap(lambda x: compute_analytical_solution_open(cdd_inv=cdd_inv, cgd=cgd, vg=x))
