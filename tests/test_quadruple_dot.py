...
def quadruple_dot_matrices():
    cdd_inv = np.eye(4) + np.random.uniform(-0.5, 0.5, size=(4, 4))
    cdd_inv = (cdd_inv + cdd_inv.T) / 2.
    cdd_inv = np.clip(cdd_inv, 0, None)

    cdd = Cdd(np.linalg.inv(cdd_inv))
    cgd = np.eye(4) + np.random.uniform(-0.5, 0.5, size=(4, 4))
...
class DoubleDotTests(unittest.TestCase):
    def test_python_vs_rust_open(self):
        ...
        for _ in range(N_ITERATIONS):
            cdd, cdd_inv, cgd = quadruple_dot_matrices()
