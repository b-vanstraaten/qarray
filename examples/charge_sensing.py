...
cdd_non_maxwell = [[0., 0.1], [0.1, 0.], ]
cgd_non_maxwell = [[1., 0.2, 0.05], [0.2, 1., 0.05], ]

cds = [[0.02, 0.01]]

cgs = [[0.06, 0.05, 1]]

model = ChargeSensedDotArray(cdd_non_maxwell=cdd_non_maxwell, cgd_non_maxwell=cgd_non_maxwell, cds_non_maxwell=cds,
                             cgs_non_maxwell=cgs, gamma=0.05, noise=0.001, threshold=1., core='r', T=0.01)

voltage_composer = GateVoltageComposer(n_gate=model.n_gate)

ground_state_funcs = [model.charge_sensor_open, partial(model.charge_sensor_closed, n_charge=1),
