...
def benchmark(core, state, n_dots, n_voltage_points, n_model_max, t_max, plot=True, save=True):
    ...
    if save:
        np.savez(f'./benchmark_data/{core}_{state}.npz', n_dots=n_dots, times=times)
    return times
...
benchmark_combinations = [
    ...
    # ('python', 'closed', np.arange(12, 1, -1)),
    # ('jax_brute_force', 'open', np.arange(7, 1, -1)),
    # ('jax_brute_force', 'closed', np.arange(7, 1, -1)),
    # ('python_brute_force', 'open', np.arange(6, 1, -1)),
    ('python_brute_force', 'closed', np.arange(6, 1, -1)),
]
